...
const AdminHeroSection2Editor = () => {
  const [form, setForm] = useState<HeroSection2 | null>(null);
  const [loading, setLoading] = useState(true);

  const fetchHeroSection2 = async () => {
    try {
      const { data } = await axios.get<HeroSection2>('http://localhost:5000/api/herosection2');
      setForm(data);
    } catch (err) {
  ...
  const initHeroSection2 = async () => {
    try {
      const { data } = await axios.post<HeroSection2>('http://localhost:5000/api/herosection2/init');
  ...
  const resetToDefaults = async () => {
    try {
      const { data } = await axios.post<HeroSection2>('http://localhost:5000/api/herosection2/reset-defaults');
      setForm(data);
