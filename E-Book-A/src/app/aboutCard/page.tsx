...
const AdminAboutUsCardsEditor = () => {
  const [data, setData] = useState<AboutUsCards | null>(null);
  ...
  const handleChange = (field: keyof AboutUsCards, value: string) => {
    if (!data) return;
  ...
  const renderCard = (num: 1 | 2 | 3) => {
    const prefix = `card${num}` as const;
    return (
  ...
  return (
    <div className="p-6 max-w-4xl mx-auto space-y-6 bg-white">
      ...
      {/* Render 3 Cards */}
      {[1, 2, 3].map((n) => renderCard(n as 1 | 2 | 3))}
